...
// CacheMusic get data from mc
func (d *dao) CacheMusic(c context.Context, id int64) (res *model.Music, err error) {
	key := keyArt(id)
	res = &model.Music{}
	if err = d.mc.Get(c, key).Scan(res); err != nil {
		res = nil
	...
	if err != nil {
		log.Errorv(c, log.KV("CacheMusic", fmt.Sprintf("%+v", err)), log.KV("key", key))
		return
	}
	return
}

// AddCacheMusic Set data to mc
func (d *dao) AddCacheMusic(c context.Context, id int64, val *model.Music) (err error) {
	if val == nil {
		return
	}
	key := keyArt(id)
	item := &memcache.Item{Key: key, Object: val, Expiration: d.demoExpire, Flags: memcache.FlagJSON}
	if err = d.mc.Set(c, item); err != nil {
		log.Errorv(c, log.KV("AddCacheMusic", fmt.Sprintf("%+v", err)), log.KV("key", key))
		return
	}
	return
}

// DeleteMusicCache delete data from mc
func (d *dao) DeleteMusicCache(c context.Context, id int64) (err error) {
	key := keyArt(id)
	if err = d.mc.Delete(c, key); err != nil {
		if err == memcache.ErrNotFound {
			err = nil
		...
		log.Errorv(c, log.KV("DeleteMusicCache", fmt.Sprintf("%+v", err)), log.KV("key", key))
		return
	}
	return
}

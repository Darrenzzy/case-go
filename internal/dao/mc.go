...
type _mc interface {
	...
	// mc: -key=keyArt -type=get
	CacheMusic(c context.Context, id int64) (*model.Music, error)
	// mc: -key=keyArt -expire=d.demoExpire
	AddCacheMusic(c context.Context, id int64, art *model.Music) (err error)
	// mc: -key=keyArt
	DeleteMusicCache(c context.Context, id int64) (err error)
}
